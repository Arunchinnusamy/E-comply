package com.example.e_comply.feature.database
...
class ComplianceRepository @Inject constructor(
    ...
    suspend fun getInspectorAnalytics(): Result<com.example.e_comply.data.remote.InspectorAnalytics> {
        return try {
            val response = apiService.getInspectorAnalytics()
            if (response.isSuccessful && response.body() != null) {
    ...
    suspend fun downloadReportPdf(reportId: String): Result<java.io.InputStream> {
        return try {
